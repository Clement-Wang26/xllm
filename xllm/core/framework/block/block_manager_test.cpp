...
TEST(BlockManagerPoolTest,
     PublishFullBlocksClampsToSequenceTokensWhenBudgetIsOverestimated) {
  ...
  EXPECT_NO_FATAL_FAILURE(
      pool.publish_full_blocks_to_prefix_cache(sequence, /*num_tokens=*/8));
  EXPECT_EQ(sequence->published_prefix_blocks_num(), 1);
