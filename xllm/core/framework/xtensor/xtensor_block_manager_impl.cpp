...
void XTensorBlockManagerImpl::free_blocks(const std::vector<int32_t>& indices) {
  ...
  if (!pages_to_free.empty()) {
    page_allocator.free_kv_cache_pages(dp_rank_, pages_to_free);
  }
}
...
void XTensorBlockManagerImpl::deallocate(const Slice<Block>& blocks) {
  ...
  for (const auto& block : blocks) {
    if (block.is_valid() && block.ref_count() == 1) {
      indices.push_back(block.id());
    }
  }
  free_blocks(indices);
}

void XTensorBlockManagerImpl::deallocate(std::vector<Block>& blocks) {
  deallocate(Slice<Block>(blocks.data(), blocks.size()));
  blocks.clear();
}
...
float XTensorBlockManagerImpl::get_gpu_cache_usage_perc() const {
  size_t total = num_total_blocks();
  if (total == 0) return 0.0f;
  return 1.0f - static_cast<float>(num_free_blocks()) / total;
}
