...
void NpuDeepseekV2DecoderLayerImpl::load_state_dict(
  ...
  LOG(INFO)
      << "test11wy. "
      << at_host_weight_tensors_[IN_BLOCK_SPARSE_MOE_GATE_WEIGHT].defined()
      << "layer_id_" << layer_id_;
}
...
void NpuDeepseekV2DecoderLayerImpl::process_general_weights(
  ...
  at_host_weight_tensors_[index] = tmp_tensor;
  LOG(INFO) << "index" << index << at_host_weight_tensors_[index].defined();
}
...
void NpuDeepseekV2DecoderLayerImpl::merge_loaded_at_weights() {
  ...
  at_host_weight_tensors_[IN_KV_PROJ_WITH_MQA_WEIGHT] = torch::zeros({1});
  at_host_weight_tensors_[IN_KV_PROJ_WITH_MQA_BIAS] = torch::zeros({1});
  at_host_weight_tensors_[IN_KV_PROJ_WITH_MQA_DESCALE] = torch::zeros({1});
  at_host_weight_tensors_[IN_KV_PROJ_WITH_MQA_OFFSET] = torch::zeros({1});
  at_host_weight_tensors_[IN_KV_PROJ_WITH_MQA_SCALE] = torch::zeros({1});
  if (FLAGS_expert_parallel_degree != 2) {
    LOG(INFO)
        << "test7wy. "
        << at_host_weight_tensors_[IN_BLOCK_SPARSE_MOE_GATE_WEIGHT].defined();
...
void NpuDeepseekV2DecoderLayerImpl::merge_experts_weights() {
  torch::Tensor mlp_gateup_weight =
      merge_experts_weights(experts_weights_["gate_proj.weight"],
                            ...
                            at::kCPU,
                            /*transpose=*/true);
  LOG(INFO) << "mlp_gateup_weight size: " << mlp_gateup_weight.sizes()
            << " layer_id_" << layer_id_;
