...
SuffixDecodingCache::SuffixDecodingCache(int max_tree_depth,
                                         int max_cached_requests)
...
SuffixDecodingDraft SuffixDecodingCache::speculate(
    ...
    std::optional<int> max_spec_tokens,
  ...
  int max_tokens = max_spec_tokens.value_or(max_tree_depth_);
...
void SuffixDecodingCache::maybe_evict_requests(int32_t new_seq_id) {
  ...
  while (static_cast<int>(req_to_seq_id_.size()) > max_cached_requests_) {
    bool evicted = false;
