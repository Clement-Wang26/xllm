...
class SuffixDecodingCache {
 public:
  SuffixDecodingCache(int max_tree_depth = 64, int max_cached_requests = -1);

  int max_tree_depth() const { return max_tree_depth_; }

  int max_cached_requests() const { return max_cached_requests_; }
  ...
  SuffixDecodingDraft speculate(
      ...
      std::optional<int> max_spec_tokens = std::nullopt,
