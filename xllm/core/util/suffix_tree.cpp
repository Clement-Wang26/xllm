...
void SuffixTree::remove(int seq_id) {
  ...
  for (int start = 0; start < static_cast<int>(seq.size()); start++) {
    Node* node = _root.get();
    node->count--;
    int idx = start;
    path.clear();
    // Loop through the nodes for this suffix.
    while (idx < static_cast<int>(seq.size())) {
      int token = seq[idx];
...
Draft SuffixTree::speculate(std::span<const int32_t> context,
                            ...
                            bool use_tree_spec) {
  Draft best_draft;
  for (int match_len = 1; match_len < static_cast<int>(context.size());
       match_len++) {
    auto [node, idx] =
        _match_context(context.subspan(context.size() - match_len, match_len));
    if (node == nullptr) {
      break;
    }
...
std::string SuffixTree::check_integrity() {
  ...
  for (int seq_id = 0; seq_id < static_cast<int>(_seqs.size()); seq_id++) {
    const std::vector<int32_t>& seq = _seqs[seq_id];
    // Loop through all suffix starting indices.
    for (int start = 0; start < static_cast<int>(seq.size()); start++) {
      int idx = start;
      ...
      Node* node = _root.get();
      visit_count[node]++;
      while (idx < static_cast<int>(seq.size()) && idx - start < _max_depth) {
        ...
        CHECK_OR_RETURN(idx + node->length <= static_cast<int>(seq.size()));
...
std::string SuffixTree::_check_node_integrity(Node* node) {
  ...
  if (node == _root.get()) {
    ...
    CHECK_OR_RETURN(node->ref_idx + node->length <=
                    static_cast<int>(_seqs[node->ref_seq].size()));
    ...
    for (auto [seq_id, end_idx] : node->endpoints) {
      ...
      CHECK_OR_RETURN(end_idx > 0 &&
                      end_idx <= static_cast<int>(_seqs[seq_id].size()));
...
Draft SuffixTree::_speculate_path(Node* node,
                                  int idx,
                                  int max_spec_tokens,
                                  float min_token_prob) {
  Draft ret;
...
Draft SuffixTree::_speculate_tree(Node* node,
                                  int idx,
                                  int max_spec_tokens,
                                  float min_token_prob) {
  Draft ret;
